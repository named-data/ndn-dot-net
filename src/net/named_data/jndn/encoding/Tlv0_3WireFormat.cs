...
namespace net.named_data.jndn.encoding {
	...
	/// </summary>
	///
	public class Tlv0_3WireFormat : WireFormat {
		/// <summary>
		...
		public override Signature decodeSignatureInfoAndValue(ByteBuffer signatureInfo,
			...
			Tlv0_3WireFormat.SimpleSignatureHolder  signatureHolder = new Tlv0_3WireFormat.SimpleSignatureHolder ();
		...
		public override Blob encodeEncryptedContent(EncryptedContent encryptedContent) {
			...
			Tlv0_3WireFormat.encodeKeyLocator(net.named_data.jndn.encoding.tlv.Tlv.KeyLocator,
		...
		public override void decodeEncryptedContent(EncryptedContent encryptedContent,
			...
			Tlv0_3WireFormat.decodeKeyLocator(net.named_data.jndn.encoding.tlv.Tlv.KeyLocator,
		...
		public override Blob encodeEncryptedContentV2(EncryptedContent encryptedContent) {
			...
			if (encryptedContent.getKeyLocator().getType() == net.named_data.jndn.KeyLocatorType.KEYNAME)
				Tlv0_3WireFormat.encodeName(encryptedContent.getKeyLocator()
		...
		public static Tlv0_3WireFormat get() {
			return instance_;
		}
		...
		/// </summary>
		///
		protected static internal void encodeSelectors(Interest interest, TlvEncoder encoder) {
			int saveLength = encoder.getLength();
		...
		/// <param name="delegationSet">The DelegationSet object to encode.</param>
		/// <param name="encoder">The TlvEncoder to receive the encoding.</param>
		protected static internal void encodeDelegationSet(DelegationSet delegationSet,
				TlvEncoder encoder) {
			// Encode backwards.
		...
		/// <param name="signedPortionEndOffset">name component and ends just before the final name component (which is assumed to be a signature for a signed interest).</param>
		/// <returns>A Blob containing the encoding.</returns>
		protected static internal Blob encodeInterestV03(Interest interest,
				int[] signedPortionBeginOffset, int[] signedPortionEndOffset) {
		...
		private static Tlv0_3WireFormat instance_ = new Tlv0_3WireFormat();
		protected static internal bool didCanBePrefixWarning_ = false;
	}
}
