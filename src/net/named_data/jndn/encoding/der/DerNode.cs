...
namespace net.named_data.jndn.encoding.der {
	...
	public class DerNode {
		...
		/// </summary>
		///
		/// <param name="nodeType">The DER node type.</param>
		...
		public static DerNode parse(ByteBuffer inputBuf, int startIdx) {
			...
			DerNode newNode;
			if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.Boolean.getNumericType())
				newNode = new DerNode.DerBoolean ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.Integer.getNumericType())
				newNode = new DerNode.DerInteger ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.BitString.getNumericType())
				newNode = new DerNode.DerBitString ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.OctetString.getNumericType())
				newNode = new DerNode.DerOctetString ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.Null.getNumericType())
				newNode = new DerNode.DerNull ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.ObjectIdentifier.getNumericType())
				newNode = new DerNode.DerOid ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.Sequence.getNumericType())
				newNode = new DerNode.DerSequence ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.PrintableString.getNumericType())
				newNode = new DerNode.DerPrintableString ();
			else if (nodeType == net.named_data.jndn.encoding.der.DerNodeType.GeneralizedTime.getNumericType())
		...
		private static DerNodeType fromNumericNodeType(int type) {
			DerNodeType[] array = (DerNodeType[]) Enum.GetValues(typeof(DerNodeType));
			for (int i = 0; i < array.Length; ++i) {
				if (array[i].getNumericType() == type)
					return array[i];
			}
	
			return  default(DerNodeType)/* was: null */;
		}
		...
		public class DerStructure : DerNode {
			...
			///
			/// <param name="nodeType">The DER node type.</param>
			public DerStructure(DerNodeType nodeType) : base(nodeType) {
		...
		public class DerByteString : DerNode {
			...
			/// <param name="nodeType">The specific DER node type.</param>
			public DerByteString(ByteBuffer inputData, DerNodeType nodeType) : base(nodeType) {
				if (inputData != null) {
