...
namespace net.named_data.jndn.tests.unit_tests {
	...
	public class TestNameMethods {
		...
		private static readonly ByteBuffer TEST_NAME_PARAMETERS_DIGEST = toBuffer(new int[] {
				0x7,
				0x36, // Name
				0x8,
				0x5, // NameComponent
				...
				0x70, 0x72, 0x65, 0x66, 0x69, 0x78,
				0x02,
				0x20, // ParametersSha256DigestComponent
				0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,
				0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
				0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f });
		...
		public void testEncodeDecode() {
			...
			try {
				...
				Assert.Fail("Can't decode TEST_NAME_IMPLICIT_DIGEST");
			}
			...
			Blob encoding3 = name3.wireEncode(net.named_data.jndn.encoding.TlvWireFormat.get());
			Assert.AssertTrue(encoding3
					.equals(new Blob(TEST_NAME_PARAMETERS_DIGEST, false)));
	
			Name decodedName3 = new Name();
			try {
				decodedName3.wireDecode(
						new Blob(TEST_NAME_PARAMETERS_DIGEST, false),
						net.named_data.jndn.encoding.TlvWireFormat.get());
			} catch (EncodingException ex_1) {
				Assert.Fail("Can't decode TEST_NAME_PARAMETERS_DIGEST");
			}
			Assert.AssertEquals(decodedName3, name3);
		}
		...
		public void testParametersSha256Digest() {
			Name name = new Name();
			...
			Assert.AssertEquals(true, name.get(0).isParametersSha256Digest());
			Assert.AssertEquals(false, name.get(2).isParametersSha256Digest());
